...
public:
  ...
  explicit condition(const std::string &condstr) {
    std::istringstream ss(condstr);
    std::string strcomp;
    ss >> reg;
    ss >> strcomp;
    if (strcomp.compare("<") == 0)
      this->compare = comp::lt;
    if (strcomp.compare(">") == 0)
      this->compare = comp::gt;
    if (strcomp.compare("<=") == 0)
      this->compare = comp::let;
    if (strcomp.compare(">=") == 0)
      this->compare = comp::get;
    if (strcomp.compare("==") == 0)
      this->compare = comp::eq;
    if (strcomp.compare("!=") == 0)
      this->compare = comp::neq;
